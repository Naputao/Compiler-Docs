...
Begin
	if 第一个操作数是值 then
		获取第一个操作数的值num
		获取要分配的数组名称array
		在局部变量空间中给array分配num个空间,将空间开头地址保存到cur_varmap中
		将下个可利用的地址设置为
		生成riscv指令[sw t0,offset1(s0)]
		设置array为某个局部变量空间的开头位置,令offset0=数组开头地址到栈顶s0的距离,
		将array保存到局部变量空间中，作为数组的开头地址指针,令offset1=array指针保存的地址到栈顶s0的距离
		生成riscv指令[addi t0,s0,offset0]
		生成riscv指令[sw t0,offset1(s0)]
	else then
		获取第一个操作数的寄存器reg0
		[lw		t0,offset(s0)]
...
Begin
	...
	if 第二个操作数是值 then
		...
		生成riscv指令[add reg1,reg1,reg2]	#sub t2,t1,t2 找到数据的存放地址，t1是数组开头位置，在高地址，所以要做减法
